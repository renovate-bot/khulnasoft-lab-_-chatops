...
module Chatops
  module Gitlab
    class TestsPipeline
      # The project where the end-to-end test full pipeline will be triggered for run against staging.gitlab.com canary
      STAGING_QUALITY_PROJECT = 'gitlab-org/quality/staging-canary'

      # The project where the end-to-end test smoke pipeline will be triggered for run against gitlab.com canary
      PRODUCTION_QUALITY_PROJECT = 'gitlab-org/quality/canary'
      ...
      def trigger_end_to_end
        ...
        trigger_variables = { feature_toggled: @feature_name,
                              feature_value: @feature_value,
                              gitlab_username: ENV.fetch('GITLAB_USER_LOGIN') }.merge(smoke_or_full)
      ...
      def ops_e2e_trigger_token
        if staging?
          # Pipeline is started against staging-canary when FF is turned ON in staging
          ENV.fetch('STAGING_CANARY_OPS_E2E_TRIGGER_TOKEN')
        elsif production?
          # Pipeline is started against canary when FF is turned ON in production
          ENV.fetch('CANARY_OPS_E2E_TRIGGER_TOKEN')
        else
      ...
      def smoke_or_full
        staging? ? { FULL_ONLY: 'true' } : { SMOKE_ONLY: 'true' }
      end
